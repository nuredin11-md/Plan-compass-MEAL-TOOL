            ...
            {['All', 'Clinical Quality & Safety', 'Resources & Capacity', 'Emergency & OPD'].map((cat) => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all cursor-pointer ${
                  filterCategory === cat ? 'bg-white text-slate-800 shadow-sm font-bold' : 'text-slate-500 hover:text-slate-800'
                }`}
              >
                {cat}
              </button>
                  ...
                  {showConnectModal && (
                    ...
                    >
                      ...
                      <p className="leading-relaxed">
                        By clicking <strong>Connect Google Keep Client</strong>, the application attempts authentication using standard consumer client IDs.
                        Because Google Keep REST API is heavily restricted to enterprises, the app maintains an <strong>Offline Ledger System</strong> which is fully encrypted and stored locally in your browser workspace!
                        This ensures you can seamlessly use Keep-compatible interactive sticky sheets immediately without a GCP credential configuration.
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[290px] overflow-y-auto pr-1">
                      {filteredKeepNotes.map((note) => {
                        ...
                        return (
                          <div
                            key={note.id}
                            className={`p-3.5 rounded-xl border shadow-sm flex flex-col justify-between gap-2.5 transition-all relative ${
                              colorBgClasses[note.color]
                            } hover:shadow-md`}
                          >
                            <div>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span
                          className={`font-mono font-bold text-sm ${
                            scoring.status === 'GAP' ? 'text-rose-600' : 'text-emerald-700'
                          }`}
                        >
